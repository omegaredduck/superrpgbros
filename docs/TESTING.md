...
| 7 | P1 | 2026-07-12 | `drinkPotion` (scenes.js) recomputes live stats via `SIM.statsFor(cls, level, potionsDrunk)` WITHOUT the equipment 4th arg — drinking a potion in the nexus silently drops all gear bonuses from the live player stats until the next realm entry / gear change re-derives them | OPEN (found in the m3q code audit) — fix: pass `CURRENT.equipment` like `applyEquipmentChange`/`grantXp` do |
| 8 | P2 | 2026-07-12 | Nexus one-overlay guards miss the bestiary: `handlePortals` checks `!vaultUi && !gyUi && !consoleUi` (no `bestiaryUi`), and `handleConsole`'s guard set omits it too — SPACE near the portal/machine could act while the bestiary is open (mitigated today only by station distance) | OPEN (found in the m3q code audit) — add `bestiaryUi` to both guard sets |
| 9 | P2 | 2026-07-12 | Bestiary page-flip uses hardcoded LEFT/RIGHT arrow codes, not BINDS — since arrows are the default movement ALTs, an arrow press while the bestiary is open both pages AND feeds movement intent; a player who rebinds arrows away still flips pages with them. Also: the static HTML fallback footer (pre-JS, or if BINDS fails) still reads the old controls line | OPEN (found in the m3q code audit) — route bestiary nav through BINDS (or suppress movement while open); refresh the static footer text |
